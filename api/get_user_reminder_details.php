...
try {
    ...
    // Se añade `password_encrypted IS NOT NULL AS has_password` para consistencia con la API de listado.
    $stmt = $pdo->prepare(
        "SELECT id, type, title, username, notes, reminder_datetime, password_encrypted IS NOT NULL AS has_password 
         FROM user_reminders 
