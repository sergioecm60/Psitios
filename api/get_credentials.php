...
try {
    ...
    $stmt = $pdo->prepare(
        "SELECT s.username, s.password_encrypted, s.iv, s.url, s.notes
         FROM sites s
    ...
    $decrypted_password = '';
    if (!empty($site_details['password_encrypted']) && !empty($site_details['iv'])) {
        $decrypted_password = decrypt_from_parts($site_details['password_encrypted'], $site_details['iv']);
